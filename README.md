...
- **QEMU Integration**: Auto-detects bundled or system QEMU, one-click start/stop
- **Environment Check**: Pre-start environment validation; auto-downloads QEMU when missing (Windows)
...
**QEMU** (app will auto-download on first run if missing)

- **macOS**: Uses Homebrew (`brew install qemu`) or bundled copy
- **Windows**: Auto-downloads portable QEMU (~250MB) when first started; or install from [qemu.org](https://www.qemu.org/download/) and add to PATH
...
- **QEMU 集成**：自动检测内置或系统 QEMU，一键启动/关闭
- **环境审查**：启动前完整检查运行环境，缺失时自动安装（如 Windows 自动下载 QEMU）
...
- **macOS**：使用 Homebrew（`brew install qemu`）或内置副本
- **Windows**：首次启动时自动下载便携版 QEMU（约 250MB）；或从 [qemu.org](https://www.qemu.org/download/) 安装并配置 PATH
